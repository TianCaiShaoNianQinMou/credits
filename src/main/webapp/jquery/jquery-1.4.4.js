 ...
 *
 * Date: Thu Nov 11 19:04:53 2018 -0500
 */
(function(window, undefined) {
	...
	var jQuery = (function() {
						...
						if (match[1]) {
							...
							if (ret) {
								...
								ret = jQuery.buildFragment([ match[1] ],
										[ doc ]);
								selector = (ret.cacheable ? ret.fragment
										.cloneNode(true) : ret.fragment).childNodes;
							}
		...
		jQuery
				.extend({
					...
					// collection
					// The value/s can be optionally by executed if its a
					// function
	...
	(function() {
		...
		var root = document.documentElement, script = document
				.createElement("script"), div = document.createElement("div"), id = "script"
				+ jQuery.now();

		div.style.display = "none";
		div.innerHTML = "   <link/><table></table><a href='/a' style='color:red;float:left;opacity:.55;'>a</a><input type='checkbox'/>";

		var all = div.getElementsByTagName("*"), a = div
				.getElementsByTagName("a")[0], select = document
	...
	jQuery
			.extend({
				...
				data : function(elem, name, data) {
					...
					var isNode = elem.nodeType, id = isNode ? elem[jQuery.expando]
							: null, cache = jQuery.cache, thisCache;
					...
					return typeof name === "string" ? thisCache[name]
							: thisCache;
				},
				...
				removeData : function(elem, name) {
					...
					var isNode = elem.nodeType, id = isNode ? elem[jQuery.expando]
							: elem, cache = jQuery.cache, thisCache = isNode ? cache[id]
							: id;
			...
			.extend({
				...
				addClass : function(value) {
					if (jQuery.isFunction(value)) {
						return this.each(function(i) {
							...
							self.addClass(value.call(this, i, self
									.attr("class")));
						});
					}
				...
				removeClass : function(value) {
					if (jQuery.isFunction(value)) {
						return this.each(function(i) {
							var self = jQuery(this);
							self.removeClass(value.call(this, i, self
									.attr("class")));
						});
					}
				...
				toggleClass : function(value, stateVal) {
					...
					if (jQuery.isFunction(value)) {
						return this.each(function(i) {
							var self = jQuery(this);
							self.toggleClass(value.call(this, i, self
									.attr("class"), stateVal), stateVal);
						});
					}
					...
					return this
							.each(function(i) {
								...
								if (jQuery.isArray(val)
										&& rradiocheck.test(this.type)) {
									this.checked = jQuery.inArray(self.val(),
											val) >= 0;
				...
				attr : function(elem, name, value, pass) {
					...
					if (name === "selected" && !jQuery.support.optSelected) {
						...
						if (parent) {
							...
							// Make sure that it also works with optgroups, see
							// #5701
					...
					if ((name in elem || elem[name] !== undefined) && notxml
							&& !special) {
						if (set) {
					...
					if (!elem.attributes[name]
							&& (elem.hasAttribute && !elem.hasAttribute(name))) {
						return undefined;
					}
	...
	var rnamespaces = /\.(.*)$/, rformElems = /^(?:textarea|input|select)$/i, rperiod = /\./g, rspace = / /g, rescape = /[^\w\s.|`]/g, fcleanup = function(
			nm) {
	...
	jQuery.event = {
		...
		add : function(elem, types, handler, data) {
			...
			// Use a key less likely to result in collisions for plain JS
			// objects.
			// Fixes bug #7150.
			...
			if (!eventHandle) {
				elemData.handle = eventHandle = function() {
					...
					return typeof jQuery !== "undefined"
							...
							.apply(eventHandle.elem, arguments) : undefined;
				};
			}
		...
		remove : function(elem, types, handler, pos) {
			...
			var ret, type, fn, j, i = 0, all, namespaces, namespace, special, eventType, handleObj, origType, eventKey = elem.nodeType ? "events"
					: "__events__", elemData = jQuery.data(elem), events = elemData
					&& elemData[eventKey];
			...
			while ((type = types[i++])) {
				...
				if (eventType.length === 0 || pos != null
						&& eventType.length === 1) {
					if (!special.teardown
		...
		trigger : function(event, data, elem /* , bubbling */) {
			...
			var handle = elem.nodeType ? jQuery.data(elem, "handle") : (jQuery
					.data(elem, "__events__") || {}).handle;
			...
			try {
				if (!(elem && elem.nodeName && jQuery.noData[elem.nodeName
						.toLowerCase()])) {
					if (elem["on" + type]
			...
			if (!event.isPropagationStopped() && parent) {
				...
				var old, target = event.target, targetType = type.replace(
						rnamespaces, ""), isClick = jQuery
						.nodeName(target, "a")
						&& targetType === "click", special = jQuery.event.special[targetType]
						|| {};

				if ((!special._default || special._default.call(elem, event) === false)
						&& !isClick
						&& !(target && target.nodeName && jQuery.noData[target.nodeName
								.toLowerCase()])) {

					try {
						if (target[targetType]) {
							// Make sure that we don't accidentally re-trigger
							// the onFOO events
		...
		handle : function(event) {
			var all, handlers, namespaces, namespace_re, events, namespace_sort = [], args = jQuery
					.makeArray(arguments);
		...
		fix : function(event) {
			...
			if (event.which == null
					&& (event.charCode != null || event.keyCode != null)) {
		...
		special : {
			...
			live : {
				add : function(handleObj) {
					jQuery.event.add(this, liveConvert(handleObj.origType,
					...
					.click(jQuery
							.proxy(fn,
									function(event) {
										...
										var lastToggle = (jQuery.data(this,
					...
					function(i, name) {
						jQuery.fn[name] = function(types, data, fn,
							...
							var type, i = 0, match, namespaces, preType, selector = origSelector
									|| this.selector, context = origSelector ? this
									: jQuery(this.context);

							if (typeof types === "object"
									&& !types.preventDefault) {
								for ( var key in types) {
									context[name](key, data, types[key],
											selector);
								}
							...
							while ((type = types[i++]) != null) {
								...
								if (type === "hover") {
									types.push("mouseenter" + namespaces,
								...
								if (name === "live") {
									...
									for (var j = 0, l = context.length; j < l; j++) {
										jQuery.event.add(context[j], "live."
												+ liveConvert(type, selector),
												{
													data : data,
													selector : selector,
													handler : fn,
	...
	function liveHandler(event) {
		var stop, maxLevel, related, match, handleObj, elem, j, i, l, data, close, namespace, ret, elems = [], selectors = [], events = jQuery
				.data(this, this.nodeType ? "events" : "__events__");
			...
			.each(
					("blur focus focusin focusout load resize scroll unload click dblclick "
						...
						jQuery.fn[name] = function(data, fn) {
							...
							return arguments.length > 0 ? this.bind(name, data,
	...
	jQuery.fn.extend({
		...
		closest : function(selectors, context) {
			...
			if (jQuery.isArray(selectors)) {
				...
				if (cur && selectors.length) {
					for (i = 0, l = selectors.length; i < l; i++) {
						...
						if (!matches[selector]) {
							matches[selector] = jQuery.expr.match.POS
									.test(selector) ? jQuery(selector, context
					...
					while (cur && cur.ownerDocument && cur !== context) {
						for (selector in matches) {
							...
							if (match.jquery ? match.index(cur) > -1 : jQuery(
									cur).is(match)) {
								ret.push({
									selector : selector,
									elem : cur,
			...
			var pos = POS.test(selectors) ? jQuery(selectors, context
					|| this.context) : null;
		...
		add : function(selector, context) {
			var set = typeof selector === "string" ? jQuery(selector, context
					|| this.context) : jQuery.makeArray(selector), all = jQuery
					.merge(this.get(), set);
	...
	var rinlinejQuery = / jQuery\d+="(?:\d+|null)"/g, rleadingWhitespace = /^\s+/, rxhtmlTag = /<(?!area|br|col|embed|hr|img|input|link|meta|param)(([\w:]+)[^>]*)\/>/ig, rtagName = /<([\w:]+)/, rtbody = /<tbody/i, rhtml = /<|&#?\w+;/, rnocache = /<(?:script|object|embed|option|style)/i,
					...
					var ret = this
							.map(function() {
								if (!jQuery.support.noCloneEvent
									...
									// IE copies events bound via attachEvent
									// when
									// using cloneNode. Calling detachEvent on
									// the
									// clone will also remove the events from
									// the orignal
									// In order to get around this, we use
									// innerHTML.
									// Unfortunately, this means some
									// modifications to
									// attributes in IE that are actually only
									// stored
									// as properties will not be copied (such as
									// the
									// the name attribute on an input).
									var html = this.outerHTML, ownerDocument = this.ownerDocument;

									if (!html) {
										var div = ownerDocument
												.createElement("div");
				...
				replaceWith : function(value) {
					if (this[0] && this[0].parentNode) {
						...
						return this.pushStack(
								jQuery(jQuery.isFunction(value) ? value()
				...
				domManip : function(args, table, callback) {
					...
					if (this[0]) {
						...
						if (first) {
							...
							for (var i = 0, l = this.length; i < l; i++) {
								callback.call(table ? root(this[i], first)
										: this[i], i > 0 || results.cacheable
										|| this.length > 1 ? fragment
	...
	jQuery.buildFragment = function(args, nodes, scripts) {
		var fragment, cacheable, cacheresults, doc = (nodes && nodes[0] ? nodes[0].ownerDocument
				|| nodes[0]
				: document);
	...
	var ralpha = /alpha\([^)]*\)/i, ropacity = /opacity=([^)]*)/, rdashAlpha = /-([a-z])/ig, rupper = /([A-Z])/g, rnumpx = /^-?\d+(?:px)?$/i, rnum = /^-?\d/,
				...
				style : function(elem, name, value, extra) {
					...
					if (value !== undefined) {
						...
						if (!hooks
							...
							// Wrapped to prevent IE from throwing errors when
	...
	if (!jQuery.support.opacity) {
		jQuery.cssHooks.opacity = {
			...
			set : function(elem, value) {
				...
				var opacity = jQuery.isNaN(value) ? "" : "alpha(opacity="
						+ value * 100 + ")", filter = style.filter || "";

				style.filter = ralpha.test(filter) ? filter.replace(ralpha,
						opacity) : style.filter + ' ' + opacity;
			}
		};
	}
	...
	if (document.documentElement.currentStyle) {
		currentStyle = function(elem, name) {
			var left, rsLeft, ret = elem.currentStyle
	...
	function getWH(elem, name, extra) {
		var which = name === "width" ? cssWidth : cssHeight, val = name === "width" ? elem.offsetWidth
				: elem.offsetHeight;
		...
		jQuery
				.each(which,
						function() {
							...
							if (extra === "margin") {
								val += parseFloat(jQuery.css(elem, "margin"
										+ this)) || 0;
	...
	if (jQuery.expr && jQuery.expr.filters) {
		jQuery.expr.filters.hidden = function(elem) {
			...
			return (width === 0 && height === 0)
					|| (!jQuery.support.reliableHiddenOffsets && (elem.style.display || jQuery
	...
	var jsc = jQuery.now(), rscript = /<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, rselectTextarea = /^(?:select|textarea)/i, rinput = /^(?:color|date|datetime|email|hidden|month|number|password|range|search|tel|text|time|url|week)$/i, rnoContent = /^(?:GET|HEAD)$/, rbracket = /\[\]$/, jsre = /\=\?(&|$)/, rquery = /\?/, rts = /([?&])_=[^&]*/, rurl = /^(\w+:)?\/\/([^\/?#]+)/, r20 = /%20/g, rhash = /#.*$/,
	...
	jQuery
			.extend({
				...
				ajax : function(origSettings) {
					var s = jQuery.extend(true, {}, jQuery.ajaxSettings,
							origSettings), jsonp, status, data, type = s.type
							.toUpperCase(), noContent = rnoContent.test(type);

					s.url = s.url.replace(rhash, "");

					// Use original (not extended) context object if it was
					// provided
					...
					// If data is available, append data to url for GET/HEAD
					// requests
					if (s.data && noContent) {
					...
					var parts = rurl.exec(s.url), remote = parts
							&& (parts[1]
									&& parts[1].toLowerCase() !== location.protocol || parts[2]
					...
					// Firefox 3
					try {
						// Set content-type if data specified and content-body
					...
					// Override the abort handler, if we can (IE 6 doesn't allow
					// it, but that's OK)
				...
				show : function(speed, easing, callback) {
					...
					if (speed || speed === 0) {
						...
						for (var i = 0, j = this.length; i < j; i++) {
							...
							// Reset the inline display of this element to learn
							// if it is
							// being hidden by cascaded rules or not
							if (!jQuery.data(elem, "olddisplay")
									&& display === "none") {
								display = elem.style.display = "";
							}
						...
						for (i = 0; i < j; i++) {
							...
							if (display === "" || display === "none") {
								elem.style.display = jQuery.data(elem,
										"olddisplay")
										|| "";
							}
						}
				...
				animate : function(prop, speed, easing, callback) {
					...
					return this[optall.queue === false ? "each" : "queue"]
							(function() {
								...
								var opt = jQuery.extend({}, optall), p, isElement = this.nodeType === 1, hidden = isElement
										&& jQuery(this).is(":hidden"), self = this;
								...
								for (p in prop) {
									...
									if (isElement
										...
										// Record all 3 overflow attributes
										// because IE does not
										...
										if (jQuery.css(this, "display") === "inline"
											...
											if (!jQuery.support.inlineBlockNeedsLayout) {
												...
												// inline-level elements accept
												// inline-block;
												// block-level elements need to
												// be inline with layout
												if (display === "inline") {
													this.style.display = "inline-block";
			...
			_default : function(fx) {
				if (fx.elem.style && fx.elem.style[fx.prop] != null) {
					fx.elem.style[fx.prop] = (fx.prop === "width"
							|| fx.prop === "height" ? Math.max(0, fx.now)
							: fx.now)
							+ fx.unit;
				} else {
	...
	function defaultDisplay(nodeName) {
		if (!elemdisplay[nodeName]) {
			var elem = jQuery("<" + nodeName + ">").appendTo("body"), display = elem
					.css("display");
	...
	if ("getBoundingClientRect" in document.documentElement) {
		jQuery.fn.offset = function(options) {
			...
			var body = doc.body, win = getWindow(doc), clientTop = docElem.clientTop
					...
					|| jQuery.support.boxModel && docElem.scrollLeft || body.scrollLeft), top = box.top
					+ scrollTop - clientTop, left = box.left + scrollLeft
					- clientLeft;
		...
		jQuery.fn.offset = function(options) {
			...
			var computedStyle, offsetParent = elem.offsetParent, prevOffsetParent = elem, doc = elem.ownerDocument, docElem = doc.documentElement, body = doc.body, defaultView = doc.defaultView, prevComputedStyle = defaultView ? defaultView
			...
			while ((elem = elem.parentNode) && elem !== body
				...
				if (elem === offsetParent) {
					...
					if (jQuery.offset.doesNotAddBorder
							&& !(jQuery.offset.doesAddBorderForTableAndCells && rtable
									.test(elem.nodeName))) {
	...
	jQuery.offset = {
		initialize : function() {
			var body = document.body, container = document.createElement("div"), innerDiv, checkDiv, table, td, bodyMarginTop = parseFloat(jQuery
					.css(body, "marginTop")) || 0, html = "<div style='position:absolute;top:0;left:0;margin:0;border:5px solid #000;padding:0;width:1px;height:1px;'><div></div></div><table style='position:absolute;top:0;left:0;margin:0;border:5px solid #000;padding:0;width:1px;height:1px;' cellpadding='0' cellspacing='0'><tr><td></td></tr></table>";
		...
		setOffset : function(elem, options, i) {
			...
			var curElem = jQuery(elem), curOffset = curElem.offset(), curCSSTop = jQuery
					.css(elem, "top"), curCSSLeft = jQuery.css(elem, "left"), calculatePosition = (position === "absolute" && jQuery
					.inArray('auto', [ curCSSTop, curCSSLeft ]) > -1), props = {}, curPosition = {}, curTop, curLeft;

			// need to be able to calculate position if either top or left is
			// auto and position is absolute
			if (calculatePosition) {
				curPosition = curElem.position();
			}

			curTop = calculatePosition ? curPosition.top : parseInt(curCSSTop,
					10) || 0;
			curLeft = calculatePosition ? curPosition.left : parseInt(
					curCSSLeft, 10) || 0;
	...
	jQuery.fn.extend({
		position : function() {
			...
			// Get correct offsets
			offset = this.offset(), parentOffset = rroot
		...
		offsetParent : function() {
			return this.map(function() {
				...
				while (offsetParent
						&& (!rroot.test(offsetParent.nodeName) && jQuery.css(
	...
	jQuery.each([ "Height", "Width" ], function(i, name) {
		...
		jQuery.fn["inner" + name] = function() {
			return this[0] ? parseFloat(jQuery.css(this[0], type, "padding"))
					: null;
		};

		// outerHeight and outerWidth
		jQuery.fn["outer" + name] = function(margin) {
			return this[0] ? parseFloat(jQuery.css(this[0], type,
